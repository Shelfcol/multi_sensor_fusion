...
int main(int argc, char *argv[]) {
    ...
    std::shared_ptr<PoseReadSave> pose_read_save_ptr = std::make_shared<PoseReadSave>();
    ...
    std::vector<PoseData>  gnss_pose_vec;

    std::string file_path = WORK_SPACE_PATH+"/slam_data/trajectory/kitti01_tum.txt";
        ...
        if (!transform_received) {
            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0) {
                ...
                if (d_time < -0.05) {
                    ...
                    gnss_pose_vec.push_back(PoseData(imu_data.time,odometry_matrix.cast<double>()));
    ...
    pose_read_save_ptr->SaveTum(file_path,gnss_pose_vec);
    std::cout<<gnss_pose_vec.size()<<" pose saved";
