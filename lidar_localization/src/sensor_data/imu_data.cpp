...
bool IMUData::SyncData(std::deque<IMUData>& UnsyncedData, std::deque<IMUData>& SyncedData, double sync_time) {
    ...
    while (UnsyncedData.size() >= 2) {
        if (UnsyncedData.front().time > sync_time) // IMU中最早的时间都比点云时间大，说明点云数据太靠前
            return false;
        if (UnsyncedData.at(1).time < sync_time) { // 说明sync_time不在一二两个IMU数据之间，则将第一个数据剔除
            UnsyncedData.pop_front();
            continue;
        }
        if (sync_time - UnsyncedData.front().time > 0.2) { // 当前时间与第一个时间差得太远，不适合做插值
            UnsyncedData.pop_front();
            return false;
        }
        if (UnsyncedData.at(1).time - sync_time > 0.2) { // 当前时间与第二个差得太远，不适合做插值
            return false;
        }
        break;
    }
