...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    }else if(registration_method == "OPTICP"){
        registration_ptr = std::make_shared<OptICPRegistration>(config_node[registration_method]);
    // }else if(registration_method == "CERESICP"){
    //     registration_ptr = std::make_shared<CeresICPRegistration>(config_node[registration_method]);
    }
     else {
...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    // 将当前点云数据cloud_data存储到current_frame_里面
    current_frame_.cloud_data.time = cloud_data.time;
    std::vector<int> indices;
    pcl::removeNaNFromPointCloud(*cloud_data.cloud_ptr, *current_frame_.cloud_data.cloud_ptr, indices); // 移除NaN

    CloudData::CLOUD_PTR filtered_cloud_ptr(new CloudData::CLOUD());
    frame_filter_ptr_->Filter(current_frame_.cloud_data.cloud_ptr, filtered_cloud_ptr); // 体素滤波降采样
    ...
    if (local_map_frames_.size() == 0) { // 第一帧数据，作为初始值，不需要进行匹配
        current_frame_.pose = init_pose_;
        UpdateWithNewFrame(current_frame_); // 将关键帧点云保存到硬盘中，更新局部地图，将关键帧位姿信息保存到global_map_frames_中
        cloud_pose = current_frame_.pose;
        return true;
    }

    // 不是第一帧，就正常匹配
    TicToc registration_tic;
    registration_ptr_->ScanMatch(filtered_cloud_ptr, predict_pose, result_cloud_ptr_, current_frame_.pose); // 当前点云，预测位姿，转换后点云，匹配得到的当前点云的全局位姿
    cloud_pose = current_frame_.pose; // 当前点云的全局位姿
    registration_tic.toc("registration");
    // 更新相邻两帧的相对运动
    step_pose = last_pose.inverse() * current_frame_.pose; // T_k+1 = T_k*dT
    ...
    if (fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) + 
        ...
        fabs(last_key_frame_pose(2,3) - current_frame_.pose(2,3)) > key_frame_distance_) { // 两帧数据距离超过阈值，则生成关键帧
...
bool FrontEnd::UpdateWithNewFrame(const Frame& new_key_frame) {
    ...
    key_frame.cloud_data.cloud_ptr.reset(new CloudData::CLOUD(*new_key_frame.cloud_data.cloud_ptr)); // 深拷贝点云数据
    CloudData::CLOUD_PTR transformed_cloud_ptr(new CloudData::CLOUD());
    
    // 更新局部地图关键帧
    //? 后面key_frame的点云数据被释放掉之后，那么保存在local_map_frames_里面的点云数据不也一样被释放掉了?
    local_map_frames_.push_back(key_frame); // 局部地图关键帧存储
    while (local_map_frames_.size() > static_cast<size_t>(local_frame_num_)) { // 如果局部地图关键帧数量超过阈值，则舍弃窗口内的第一帧数据
    ...
    for (size_t i = 0; i < local_map_frames_.size(); ++i) {
        pcl::transformPointCloud(*local_map_frames_.at(i).cloud_data.cloud_ptr, 
                                 *transformed_cloud_ptr, 
                                 local_map_frames_.at(i).pose); // 根据关键帧的点云和对应的全局位姿，将点云转到全局坐标系下，保存在transformed_cloud_ptr中
        *local_map_ptr_ += *transformed_cloud_ptr; // 全局坐标系下的点云进行叠加
    }
    ...
    if (local_map_frames_.size() < 10) {
        registration_ptr_->SetInputTarget(local_map_ptr_); // 关键帧数量太少，则表示此时点云数据比较稀疏，就不进行稀疏操作
    } else {
        CloudData::CLOUD_PTR filtered_local_map_ptr(new CloudData::CLOUD()); 
        local_map_filter_ptr_->Filter(local_map_ptr_, filtered_local_map_ptr); // 关键帧点云数据较多，则进行降采样
        registration_ptr_->SetInputTarget(filtered_local_map_ptr);
    }

    // 保存所有关键帧信息在容器里
    // 存储之前，点云要先释放，因为已经存到了硬盘里，不释放也达不到节省内存的目的
    key_frame.cloud_data.cloud_ptr.reset(new CloudData::CLOUD()); // 将关键帧点云数据释放掉
