...
// 定义顶点
class CurveFittingVertex: public g2o::BaseVertex<3,Eigen::Vector2d> // 顶点维杜，顶点数据类型
{
public:
...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    CurveFittingEdge(const double& x):x_(x){}
    void computeError(){
        // 还原这条边的顶点，这里根据继承的边确定
        ...
        _error(0) = _measurement-std::exp(ab(0)*x_+ab(1)); //_error是D维向量 Eigen::Matrix<double, D, 1, Eigen::ColMajor>
    }
    virtual bool read(istream& in){}
    virtual bool write(ostream& out)const {}
public:
