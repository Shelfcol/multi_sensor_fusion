...
struct TestSE3CostFunctor {
  ...
  bool operator()(T const* const sT_wa, T* sResiduals) const {
    Eigen::Map<Sophus::SE3<T> const> const T_wa(sT_wa); // 将数组映射为SE3
    Eigen::Map<Eigen::Matrix<T, 6, 1> > residuals(sResiduals); // 将数组映射为矩阵，残差为6*1，因为状态量为6维
...
struct TestPointCostFunctor {
  ...
  bool operator()(T const* const sT_wa, T const* const spoint_b,
                  T* sResiduals) const { // 同时优化转换矩阵和点，加上残差项
